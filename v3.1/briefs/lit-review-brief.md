...
> ⚠️ **[DATE] 真实语料核查**："新组合"是贡献类型（novel synthesis 真实存在），但"脉络A+脉络B→交叉空白"只用于**定位你的贡献**，不能直接写成 Gap 句——真实论文 Gap 几乎全是单线推进式（传统聚焦 A → 忽视/缺陷 B → 本文做 C）。Gap 句写法详见 `journals/cities/01-introduction.md` 第二节 2.2 小节。

...
> **数据基础**：Zotero 库内真实 Cities 论文"数据与方法 Data & Methods"节精提取 数十篇。全部例句逐字摘录自原文，可回库核实。
...
> **v1 遗留问题（已修正）**：①v1 把"空间技术参数清单"写成万能模板——实测数据节的内部组织由**数据源类型**决定（信令/问卷/普查/GPS/多源融合各有各的顺序），不是一套参数表；②v1 部分句子无出处，现在每条都有真实论文 tag；③v1 缺"样本代表性怎么论证""无独立数据节时谁承担数据功能"两条指导——正是手机信令、数据驱动实证类稿件最需要的。
...
这类论文的数据节段落结构在 数十篇里高度一致，逐段功能如下（**可直接照此组织**）：

...
> **语料基础**：Zotero 库内真实 Cities 论文"数据与方法 Data & Methods"节精提取 数十篇。全部例句逐字摘录自原文，可回库核实。
...
> **v1 遗留问题（已修正）**：①v1 写"方法节必须有空间方法"，实测方法节的组织由**分析链类型**决定（步骤流水线/概念框架先行/数据→变量→模型），不是"空间方法清单"；②v1 部分句子无出处，现在每条都有真实论文 tag；③v1 缺你最需要的三类句子——**时间使用/活动空间类指标构建**、**K-means 聚类交代**、**统计模型选择论证**，本版专章补齐。
...
## 二、段落级写法（按常见分析链组织：指标构建 → K-means 聚类 → 组间比较）

### 2.1 指标/变量构建（时间使用、活动空间类指标）
...
- **"不作直接解释变量"的内生性规避声明**（聚类分组作分组变量而非解释变量）：`This dual mechanism provides the theoretical basis for our empirical analysis, where mobility is used as a grouping variable rather than a direct explanatory variable, thereby avoiding potential endogeneity issues.`（2026_Liu_8807）
...
> 与 [03-data.md](03-data.md)（数据节：管"数据是什么/代表谁/局限"，方法节管"怎么分析"）配合使用；两节分工界面：识别规则（职住/停留）可放数据节，指标构建（时间使用/活动空间）放方法节。
> 语料中间产物：数十篇数据与方法节精提取、逐篇全章节精提取。

...
> **数据基础**：Zotero 库内真实 Cities 论文"结果 Results"节精提取 数十篇。全部例句逐字摘录自原文，可回库核实。
...
> **v1 遗留问题（已修正）**：①v1 把"空间格局开场→群体对比→空间统计→城市解释"写成万能范式——实测结果节的内部组织由**研究问题的推进逻辑**决定，"空间格局开场"只是其中一类论文的开场，不是签名；②v1 部分句子无出处，现在每条都有真实论文 tag；③v1 缺"聚类/分组结果怎么呈现""差异检验怎么报""表图如何呼应"三条指导——正是聚类/分组实证稿件最需要的。
...
### 1.1 描述统计 → 主发现 → 异质性/机制（常见主线，重点）

最常见的结果链（样本 → 多指标 → K-means 分组 → 组间比较）的递进：
...
- 判断：只有"发现链很短、每步一个检验"的论文才适合并入其他节；多指标+聚类+组间比较的稿件需要独立 Results 节。
...
**组间比较**（"whereas/while/in contrast" 对比，组间比较核心句）：

...
| `data-engineering/dask` | 超内存 pandas 并行处理 | **数据量**超单机内存时 | `uv pip install dask` |
...
| `coding/shap` | ML 模型解释（瀑布图/蜂群图/特征重要性） | 若用 ML 模型（非 K-means 等聚类方法）做变量重要性 | `uv pip install shap` |

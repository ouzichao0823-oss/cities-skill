...
> ⚠️ **[DATE] 真实语料核查**：上表"新组合"是**贡献类型分类**（你提供了什么贡献），不是 **Gap 句写法**。真实论文的 Gap 句几乎全是单线推进式（传统聚焦 A → 忽视/缺陷 B → 本文做 C）；"两条文献线相互承认但交汇点/从未交叉"的写法在真实语料中不存在。跨线研究（如时间使用 × 活动空间）也只以"忽视 B"单线表述。贡献声明可以说 "we bridge two strands"，但 Gap 句不能用"两线未合并"结构。Gap 写法详见 `journals/cities/01-introduction.md` 第二节 2.2 小节（[DATE] 重写）。
